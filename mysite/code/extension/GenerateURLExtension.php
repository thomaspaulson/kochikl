<?php
/**
 * Adds URLSegment functionality to TypeTags & Location.
 *
 */
class GenerateURLExtension extends DataExtension
{
    /**
     * @var array
     */
    ...
    public function generateURLSegment($increment = null)
    {
        ...
        if (!$this->owner->ContentHolderID) {
            $this->owner->ContentHolderID = 0;
        }

        $duplicate = DataList::create($this->owner->ClassName)->filter(array(
            'URLSegment' => $this->owner->URLSegment,
            'ContentHolderID' => $this->owner->ContentHolderID,
        ));
